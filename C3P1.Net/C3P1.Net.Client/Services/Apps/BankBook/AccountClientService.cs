...
namespace C3P1.Net.Client.Services.Apps.BankBook
{
    public class AccountClientService(HttpClient httpClient) : IAccountService
    {
        ...
        {
            var result = await httpClient.GetFromJsonAsync<List<Account>>("api/apps/bankbook/account/list");

            return result!;
        }
        public async Task<bool> AddAccountAsync(Guid userId, Account bankAccount)
        {
            var result = await httpClient.PostAsJsonAsync<Account>($"api/apps/bankbook/account/add", bankAccount);
        ...
        {
            var result = await httpClient.DeleteAsync($"api/apps/bankbook/account/delete/{bankAccountId}");
        ...
        {
            var result = await httpClient.PutAsJsonAsync<Account>("api/apps/bankbook/account/update", bankAccount);
        ...
        {
            var result = await httpClient.GetFromJsonAsync<Account>($"api/apps/bankbook/account/get/{bankAccountId}");
            return result;
        }

        public async Task<Account?> GetAccountByCodeAsync(Guid userId, string code)
        {
            var result = await httpClient.GetFromJsonAsync<Account>($"api/apps/bankbook/account/get/code/{code}");
            return result;
        }
    }
}
